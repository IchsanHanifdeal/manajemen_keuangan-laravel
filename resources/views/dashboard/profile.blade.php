        ...
        <div class="flex flex-col gap-5 p-5 sm:p-7 bg-white border-back rounded-xl w-full">
            <div class="flex gap-5 border-b pb-7">
                <div class="flex flex-col items-center gap-3 h-fit">
                    <div class="w-24 h-24 rounded-xl overflow-hidden border-2 border-gray-300">
                        <img class="w-full h-full object-cover rounded-xl" src="https://ui-avatars.com/api/?name={{ urlencode(session()->get('nama')) }}" />
                    </div>                    
            ...
            <form class="flex flex-col gap-3" method="POST" action="{{ route('update.profile', ['id_user' => $id_user]) }}">
                ...
                <div class="input-label">
                    ...
                    <input required name="nama" value="{{ session()->get('nama') }}" type="text" placeholder="...">
                    @error('nama')
                ...
                <div class="input-label">
                    ...
                    <input required name="username" value="{{ session()->get('username') }}" type="text" placeholder="...">
                    @error('username')
                        <span class="validated">{{ $message }}</span>
                    @enderror
                </div>
                <div class="input-label">
                    ...
                    <input disabled name="email" value="{{ session()->get('email') }}" type="email" placeholder="...">
                </div>
